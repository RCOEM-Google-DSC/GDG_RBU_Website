...
export function ViewToggleButtons({ viewMode, onViewModeChange }: ViewToggleProps) {
  return (
...
export function GridView<TData>({
  ...
  const defaultRenderCard = (item: TData, index: number) => {
    ...
    const assignee = (task.assignee as string) || (task.assignTo as string) || '';
      ...
      >
        ...
        {(status || priority) && (
          <div className="flex flex-row items-center gap-2">
            {status && (
              <span className={`text-xs px-2 py-1 rounded-md border ${getStatusStyle(status)}`}>
                {status.toLowerCase()}
              </span>
            )}
            {priority && (
              <span className={`text-xs px-2 py-1 rounded-md border ${getPriorityStyle(priority)}`}>
