...
export default function Home() {
  ...
  useEffect(() => {
    ...
    if (redTrailRef.current) redTrailRef.current.style.strokeDasharray = `${topTotalLength}`;
    if (blueTrailRef.current) blueTrailRef.current.style.strokeDasharray = `${topTotalLength}`;
    if (yellowTrailRef.current) yellowTrailRef.current.style.strokeDasharray = `${bottomTotalLength}`;
    if (greenTrailRef.current) greenTrailRef.current.style.strokeDasharray = `${bottomTotalLength}`;
    ...
    const animateFrame = (currentTime: number) => {
      ...
      if (prevRedProgress > 0.95 && redProgress < 0.05) {
        redLoopCount++;
        if (topBaseRef.current) topBaseRef.current.setAttribute("stroke", "#dc2626");
      }
      ...
      if ((currentTime - redStartTime) >= topDelay) {
        ...
        if (prevBlueProgress > 0.95 && blueProgress < 0.05) {
          blueLoopCount++;
          if (topBaseRef.current) topBaseRef.current.setAttribute("stroke", "#2563eb");
        }
        ...
        // Z-Index Top
        if (topTrailsGroupRef.current && redTrailRef.current && blueTrailRef.current) {
            ...
            if (redShouldBeTop) topTrailsGroupRef.current.appendChild(redTrailRef.current);
      ...
      if (prevYellowProgress > 0.95 && yellowProgress < 0.05) {
        yellowLoopCount++;
        if (bottomBaseRef.current) bottomBaseRef.current.setAttribute("stroke", "#eab308");
      }
      ...
      if ((currentTime - yellowStartTime) >= bottomDelay) {
        if (greenStartTime === null) {
          ...
          if (greenLogoRef.current) greenLogoRef.current.setAttribute('opacity', '1');
          if (greenTrailRef.current) greenTrailRef.current.setAttribute('opacity', '1');
        }
        ...
        if (prevGreenProgress > 0.95 && greenProgress < 0.05) {
          greenLoopCount++;
          if (bottomBaseRef.current) bottomBaseRef.current.setAttribute("stroke", "#16a34a");
        }
        ...
        // Z-Index Bottom
        if (bottomTrailsGroupRef.current && yellowTrailRef.current && greenTrailRef.current) {
            ...
            if (yellowShouldBeTop) bottomTrailsGroupRef.current.appendChild(yellowTrailRef.current);
    ...
    <div className="w-full bg-white relative overflow-hidden">
      ...
      <div className="py-10 relative">
        ...
        >
          ...
          <path d={pathD} stroke="transparent" strokeWidth="2" fill="none" vectorEffect="non-scaling-stroke" />
          <path ref={topBaseRef} d={pathD} stroke="transparent" strokeWidth="3" fill="none" vectorEffect="non-scaling-stroke" />
      ...
      <div className="-mt-32 relative">
        <svg
          ...
          style={{ overflow: 'visible' }}
        >
          <path ref={bottomPathRef} d={bottomPathD} stroke="transparent" fill="none" />
          {/* Black Line made transparent */}
          <path d={bottomPathD} stroke="transparent" strokeWidth="2" fill="none" vectorEffect="non-scaling-stroke" />
          <path ref={bottomBaseRef} d={bottomPathD} stroke="transparent" strokeWidth="3" fill="none" vectorEffect="non-scaling-stroke" />
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
         <div className="text-center bg-white/80 p-6 backdrop-blur-sm rounded-xl">
            <h1 className="text-5xl font-bold mb-4 text-black">
              Google Developer<br/>Groups, RBU
            </h1>
            <p className="text-gray-600 max-w-lg mx-auto">
              Empowering students with cutting-edge tech skills, community, and resources for a future in technology.
            </p>
         </div>
      </div>
