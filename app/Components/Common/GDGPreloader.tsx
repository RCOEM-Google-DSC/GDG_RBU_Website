...
const GDGPreloader: React.FC = () => {
  useEffect(() => {
    // store previous overflow values to restore later
    const prevHtmlOverflow = document.documentElement.style.overflow;
    const prevBodyOverflow = document.body.style.overflow;

    // Add the active class synchronously on mount
    ...
    // Remove preloader-active after the animation duration (4s)
    ...
    return () => {
      clearTimeout(timer);
      // Ensure cleanup if the component unmounts for any reason
      document.documentElement.classList.remove("preloader-active");
      document.documentElement.style.overflow = prevHtmlOverflow || "";
      document.body.style.overflow = prevBodyOverflow || "";
    };
