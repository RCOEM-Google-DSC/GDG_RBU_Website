...
const PastEvent = ({
  ...
  const finalHref = normalizedWebsite ?? `/events/${id}`;

  // Wrapper component based on link type
  const CardWrapper = ({ children }: { children: React.ReactNode }) => {
    ...
    return (
      <Link href={finalHref} className="block w-full" aria-label={`View ${title}`}>
        {children}
      </Link>
    );
  };
  ...
  return (
    <div className="w-full flex items-center justify-center">
      <CardWrapper>
        ...
        <div className="relative w-full max-w-[340px] h-[450px] drop-shadow-sm filter cursor-pointer transition-transform hover:scale-[1.02] active:scale-[0.98]">
          ...
          <div className="relative z-10 px-5 sm:px-6 pt-9 pb-8 h-full flex flex-col">
            ...
            <div className="flex items-start justify-between mb-3">
              ...
              {/* Arrow button - now just a visual indicator */}
              <Button
                className="bg-[#fbbf24] hover:bg-[#f59e0b] transition-colors rounded-full p-2 flex items-center justify-center shrink-0 pointer-events-none"
                aria-hidden="true"
              >
